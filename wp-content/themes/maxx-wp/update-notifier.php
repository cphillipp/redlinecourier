...
//Custom URL Checking to prevent slow loading when the remoted file's server down
$localXML_URL = get_template_directory_uri().'/notifier.xml';
$remoteXML_URL = 'http://dmanh.com/update/maxx/notifier.xml';
$remoteRespond = wp_remote_head( $remoteXML_URL);
$xmlData = (isset($remoteRespond['response']['code']) && $remoteRespond['response']['code'] == 200) ? $remoteXML_URL : $localXML_URL;

// Constants for the theme name, folder and remote XML url
define( 'NOTIFIER_THEME_NAME', 'Maxx' ); // The theme name
define( 'NOTIFIER_THEME_FOLDER_NAME', 'maxx-wp' ); // The theme folder name
define( 'NOTIFIER_XML_FILE', $xmlData ); // The remote notifier XML file containing the latest version of the theme and changelog
...
function update_notifier_menu() {  
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		$current_latest = str_replace(".", "", $themedata['Version']);
		
		if( (float)$xml_latest > (float)$current_latest) { // Compare current theme version with the remote XML version
			add_dashboard_page( NOTIFIER_THEME_NAME . __(' Theme Updates','framework'), NOTIFIER_THEME_NAME . ' <span class="update-plugins count-1"><span class="update-count">'.__('New Updates','framework').'</span></span>', 'administrator', 'theme-update-notifier', 'update_notifier');
		}
	}	
}
...
function update_notifier_bar_menu() {
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		if( (float)$xml->latest > (float)$themedata['Version']) { // Compare current theme version with the remote XML version
...
function update_notifier() { 
	...
	<div class="wrap">
		...
		<h2><?php echo NOTIFIER_THEME_NAME ?></h2>
	    <div id="message" class="updated below-h2"><p><strong>There is a new version of the <?php echo NOTIFIER_THEME_NAME; ?> theme available.</strong> You have version <?php echo $themedata['Version']; ?> installed. Update to version <?php echo $xml->latest; ?>.</p></div>
